...
// Resolve workspace from called number
...
// Upsert lead by normalized phone within workspace
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await logEvent(supabase, workspace_id, "lead_upserted", {
      lead_id: lead.id,
      phone: callerNumber,
    });
    ...
    if (isMissed || callStatus === "voicemail") {
      ...
      if (automationEnabled && config.account_sid && config.auth_token && config.from_number) {
        // Get workspace name for SMS template
        ...
        try {
          ...
          // Create/reset qualification session
